...
const BasketCard: React.FC<{ basket: FoBasket }> = ({ basket }) => {
    ...
    return (
        <Card className="flex flex-col shadow-md hover:shadow-lg transition-shadow">
            ...
            {isOpen && (
                <>
                    <CardContent className="space-y-4 text-sm flex-grow p-4 pt-0 animate-accordion-down">
                    ...
                    <CardFooter className="border-t mt-auto p-3 flex justify-end space-x-2 animate-accordion-down">
...
export function CryptoBasketSection() {
  const baskets = mockFoBaskets.filter(basket => basket.name.toLowerCase().includes('crypto') || basket.name.toLowerCase().includes('defi'));
  
  return (
    <Card className="shadow-md">
        <CardHeader>
            <CardTitle className="text-xl font-semibold font-headline text-primary flex items-center">
                <ShoppingBasket className="h-6 w-6 mr-2" /> Crypto Baskets
            </CardTitle>
        </CardHeader>
        <CardContent>
            {baskets.length === 0 ? (
                <p className="text-muted-foreground text-center py-8">You have no crypto baskets.</p>
            ) : (
