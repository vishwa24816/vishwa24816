...
interface FiatOptionChainProps {
...
const fiatUnderlyings = mockUnderlyings.filter(u => u.symbol !== 'BTC' && u.symbol !== 'ETH');


export function FiatOptionChain({ onAddLeg }: FiatOptionChainProps) {
  const [selectedUnderlyingSymbol, setSelectedUnderlyingSymbol] = useState<string>(fiatUnderlyings[0].symbol);
  ...
  const selectedUnderlyingDetails = useMemo(() => fiatUnderlyings.find(u => u.symbol === selectedUnderlyingSymbol), [selectedUnderlyingSymbol]);
      ...
      <div className="p-2 sm:p-3 border-b border-border flex flex-wrap items-center justify-between gap-2 shrink-0">
        <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
          <Select value={selectedUnderlyingSymbol} onValueChange={setSelectedUnderlyingSymbol}>
            ...
            <SelectContent>{fiatUnderlyings.map(u => <SelectItem key={u.id} value={u.symbol} className="text-xs">{u.name}</SelectItem>)}</SelectContent>
          </Select>
